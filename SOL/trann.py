...
class TARNN (nn.Module):
    ...
    def forward(self, inputs, h=None):
        ...
        for input in torch.unbind (inputs, dim=0):
            ...
            aa = torch.matmul (old_h, self.Ug)
            bb = torch.matmul (input, self.Wg)
            hg = torch.sigmoid(aa + bb+self.bg)

            # hg = torch.sigmoid (torch.mm (old_h, self.Ug)+ torch.mm (input, self.Wg) + self.bg)

            arg = torch.matmul (old_h, self.U2) + torch.mm (input, self.W1) + torch.mm (old_input, self.W2) + self.b1
            bx = torch.matmul (input, self.B1)
            bh = torch.matmul (h, self.B2)

            if self.b_variant:
                for k in range (self.K):
                    at1 = self.A1[k][t]
                    h = (1 - at1) * h + at1 * (self.NL (torch.matmul (h, self.U1) + arg) - h - old_h + bx + bh)

                h = (1 - hg) * old_h + hg * h
            else:
                for k in range (self.K):
                    at1 = self.A1[k][t]
                    h = h + at1 * hg * (self.NL (torch.matmul (h, self.U1) + arg) - h - old_h + bx + bh)
